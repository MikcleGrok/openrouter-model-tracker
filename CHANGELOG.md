...
- Add `--refresh` to `report`: fetches fresh data first (the exact same path `openrouter refresh` runs) and only then renders the report from the just-updated snapshot — `openrouter report --refresh` in place of `openrouter refresh && openrouter report`. Strictly opt-in: without the flag, `report` still never touches the network, byte-for-byte as before.
...
- Make `openrouter history`'s default output a real browsable table: `--format` now defaults to `report`, so a bare `omt history` prints the deduplicated, column-aligned price table — consecutive unchanged-price days collapsed into one date-range row — instead of the unaligned, tens-of-thousands-of-lines Markdown dump. Without `--model` it now aggregates every tracked model into one table with a `Slug` column (previously `--format report` refused to run without `--model`); with `--model` it still adds the two labeled bar charts. In an interactive TTY the output is piped through `less -S` for horizontal scrolling, disabled with the new `--no-pager` flag, matching `omt table`; `--format markdown`/`--format tsv` remain unchanged, byte-for-byte, for scripting and doc copy-paste. The empty-history message is now English in every format, matching the rest of the command's output.
...
- Add the `copyright_guardrail` TUI column and structured filter, including CSV combinations of `enforces`, `bypasses`, and `unknown`.
...
- Fix the Model Detail overlay's "Open weights" and "Claude reference" values (and, via License's fallback to Open weights, "License") staying hardcoded Russian in English mode: a bare "да"/"нет" Open weights boolean now renders "yes"/"no", and the `_нужен обзор_` "needs review" sentinel shown whenever a model has no `notes.yaml` entry at all now renders as `_needs review_` in both fields. Annotated Open weights values (license names, partial-support caveats) are genuinely curated free text and are deliberately left untranslated, same as License and Note.

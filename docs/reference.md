...
мейнтейнера и контрибьюторов. Общее описание проекта и рекомендуемый способ
установки — в [README.md](../README.md); альтернативные способы установки —
ниже.

- [Альтернативные способы установки](#альтернативные-способы-установки)
...
напрямую, поэтому SmartScreen-предупреждение не показывается — в отличие от
ручной установки ниже.

**Windows без пакетного менеджера** — zip-архив из
...
1.26.5+. На macOS/Linux также доступен `git clone ... && cd ... && make
install` — подробности (`PREFIX`/`BINDIR`, локальный disposable tap для
разработки) в разделе «Локальная разработка» ниже.
...
- `openrouter report [-s|--sort KEY] [--ranking=legacy|tier|mixed-utility] [--score-source=swebench|arena|general] [--output PATH] [--open] [--format=markdown|html|both] [--refresh]` —
  ...
  качеству» — «Фавориты» и tier-таблицы по-прежнему ранжируются
  `model.RankFavorites`/`model.TierRows` независимо от `--sort`. `--format
  html`/`both` пишут второй, самостоятельный HTML-файл (инлайн CSS/JS, без
  внешних зависимостей и без новых Go-модулей) с ровно теми же секциями и
  колонками, что и Markdown, плюс кликабельная сортировка «Рейтинга» и
  таблицы бесплатных моделей и внутренние ссылки обзор → проверка → аудит;
  путь HTML по умолчанию получается заменой расширения у `--output`/
  `default_output` на `.html`, а под явным `--output` вместе с `--format
  html` путь используется буквально (`--format both` всегда выводит HTML
  заменой расширения — второго флага пути для HTML нет). `--format both`
  публикует Markdown и HTML одной атомарной транзакцией, так что они не
  могут разойтись на диске. `--open` открывает записанный документ (HTML,
  если он записан, иначе Markdown) системным обработчиком; если открыть не
  удалось — предупреждение в stderr, команда всё равно завершается успешно
  (файл уже записан). Не путать с `history --format report` ниже — тот
  формат относится к таблице истории цен, а не к этой команде. При
  нестандартных `pricing.mix_input_weight`/`mix_output_weight` в конфиге
  `report` (как и `table`/`tui`) показывает Q/P, пересчитанный с учётом этих
  весов, тогда как `refresh` использует встроенный блендинг 3:1 — на
  дефолтном конфиге числа совпадают, при нестандартном между `report` и
  `refresh` возможно небольшое расхождение Q/P в одном и том же документе.
- `openrouter history [--model SLUG] [--since RFC3339|YYYY-MM-DD] [--format markdown|tsv|report] [--no-pager]` — показать историю цен; по умолчанию `--format` равен `report`. `report` без `--model` агрегирует **все** отслеживаемые модели в одну выровненную по колонкам таблицу с колонкой `Slug`: дедуплицированные по цене ряды (один ряд на отдельную цену — диапазон дат и число дней вместо повторяющейся строки на каждое ежедневное наблюдение), в порядке slug, а внутри slug — хронологически; графиков нет — сотни графиков не вид, а таблица, которую прокручивает `less -S`. `report` с `--model` не изменился: та же дедуплицированная таблица для ОДНОЙ модели плюс подписанный столбчатый график по каждому из input/output рядов с осью $ и датами; ширина графика подстраивается под реальный терминал, если stdout — TTY, иначе использует `pricehistory.DefaultChartWidth`. `markdown`/`tsv` не изменились: полный журнал изменений по всем моделям (или по одной при `--model`), построчно, байт-в-байт как раньше — для скриптов и копирования в документацию. `--since` фильтрует наблюдения до дедупликации, поэтому `From` первого ряда — это первое оставшееся после фильтра наблюдение, а не истинное начало действия этой цены. Как и у `table`, в интерактивном TTY вывод передаётся в `less -S`, если не указан `--no-pager`, а при перенаправлении в pipe или файл pager не запускается — это верно для всех трёх форматов. `make history` в интерактивном терминале теперь тоже открывает `less`, как и `make table`.
...
`model-map.tsv`). Сами они к сети не обращаются; если локального снимка ещё
нет, `table`/`tui`/`report` один раз выполняют bootstrap-`refresh` по сети
(`ensureLocalSnapshot`), а дальше работают уже полностью офлайн — отдельного
предварительного `openrouter refresh` для первого запуска не требуется. Этот
bootstrap завершается с ошибкой, только если нет ни снимка, ни доступного
`--output`/`default_output`, из которого его получить.
...
По умолчанию таблица сортируется в явном режиме `utility` с ranking `mixed-utility`: сначала идут rankable-модели, а платные модели сравниваются по безопасной YAML expression formula. Без `formula` действует value heuristic формула `score + price_weight*tier_factor*ln(1+quality_price)`, где `price_weight=10`, price mix равен 3:1, а факторы `opus=1`, `sonnet=1`, `haiku=0.5`, `free=0`. Она не меняет quality и не делает invalid observation rankable. Бесплатные rankable-модели сравниваются по score. `--sort` принимает `utility`, `name`, `slug`,
 `context`, `input`, `output`, `price`, `quality`, `q/p` и `utility`, а также `Q`, `P`, `QP`; явный `q/p` всегда сортирует по показанному raw Q/P независимо от `--ranking`, а `utility` использует выбранный ranking. `Q` означает valid quality по убыванию, `P` — price по возрастанию, `QP` — q/p по убыванию. Фильтры: `paid`, `free`, `scored`, `tier:VALUE[,VALUE...]`, `task_fit:K1,K2,...`, `task_fit:`, `quality>=N`, `context>=N`, `input<=N`, `output<=N`. `tier:VALUE[,VALUE...]` — точный фильтр по одному или нескольким tiers: например, `tier:opus,haiku` выбирает Opus или Haiku; `tier:sonnet` выбирает только Sonnet. `(any)` означает отсутствие tier-предиката. Значение `free` остаётся допустимым в CLI-синтаксисе и выбирает только бесплатные модели. Внутри одного `task_fit:` keywords объединяются через OR, повторные `task_fit:` predicates — через AND, а остальные фильтры объединяются с результатом task-fit через AND: например, `openrouter table --filter 'paid,task_fit:implement,debug' --filter 'tier:sonnet'`. Операторы: `:` для выбора значения, `>=` для минимального порога и `<=` для максимального. `quality` использует только valid exact-product observation активного источника и шкалу `0..100`; mismatch, legacy и observation-only исключаются. Для удобства допускается дробный формат `0..1`, поэтому `quality>=0.8` эквивалентно `quality>=80`. Качество и Q/P сортируются по убыванию, `--reverse`/`-R` инвертирует основной порядок, а отсутствующие или неранжируемые значения всегда остаются в конце. Затем применяется `--limit`, поэтому `-n 1` выбирает первую модель уже отсортированного результата. `--reverse` меняет основной порядок, slug
...
`docs/openrouter-model-comparison.md` по умолчанию собирается в семантике
`swebench` — так его всегда пишет `refresh`, у которого нет флага выбора
источника. `openrouter report --score-source=arena|general` перегенерирует
тот же документ офлайн в одном из двух других представлений; следующий
scheduled `refresh` вернёт файл к `swebench`-виду.
...
Иконка должна быть одним символом из диапазона `U+1F300` и выше, без селектора
вариантов (`U+FE0F`). Ниже этого диапазона emoji-представление не является
свойством кодовой точки: терминал, у которого нет глифа в моноширинном шрифте,
подставляет emoji-шрифт и рисует два знакоместа там, где любая таблица ширин
сообщает одно, — так вели себя прежние `Ⓩ`/`Ⓝ`/`ⓧ`. Для `Ⓜ️` (meta), `♟️`
(minimax), `🌪️` (mistral) несовпадение работает в противоположную сторону:
библиотеки ширин утверждали 2 знакоместа, но реальный терминал рисует их только
на 1 знакоместо, что также разъезжает колонку.
...
`ranking.mixed_utility.price` влияет только на внутренние переменные `price_mix`/`quality_price` формулы ранжирования. Показанная колонка Q/P и сортировка `price`/`q/p` берут блендированную цену из отдельного, независимого ключа:
...
По умолчанию `3`/`1` — тот же блендинг, что `MixedPrice` использовал всегда (`(3×input + output) / 4`), поэтому конфиг без этой секции ничего не меняет. Оба веса должны быть конечными, неотрицательными числами с положительной суммой — как и `ranking.mixed_utility.price`, невалидные значения (например `0`/`0` или отрицательное) отклоняются при загрузке конфигурации с ясной ошибкой.
...
`docs/assets/tui-demo.gif` — два полных прохода одного и того же сценария:
Pass 1 на английском, затем сброс к исходному состоянию, переключение языка
(`l`) и Pass 2 — тот же сценарий на русском, без возврата обратно в EN. Обе
части структурно зеркальны и совпадают с комментариями `# 1.`–`# 5.` в
`docs/assets/tui-demo.tape` (блок `PASS 1: ENGLISH`) и их зеркальными копиями
в блоке `PASS 2: RUSSIAN`:

1. список моделей с фильтром `availability:paid`
2. два сорта (`q` — quality, `r` — q/p) по полному неотфильтрованному списку,
   со сбросом курсора на верхнюю строку перед каждым
3. структурный редактор фильтра (`f`, `quality>=80` и `input<=1`), сужающий
   тот же список, что оставил последний сорт
4. все 4 вкладки Detail (`1`-`4`: Identity, Pricing, Benchmarks, Fit & Notes)
5. все 6 секций Help (`?`, затем `1`-`6`: Overview, Score Sources, Hotkeys,
   Filters, Model Detail, Methodology)

Между проходами — блок `RESET` (`s`/`f`/`c`/Down 9/Right 3/Enter/`g`):
возвращает список к тому же состоянию, с которого начинался Pass 1 (423
модели, `availability:paid`, сорт `utility`), иначе Pass 2 сужал бы уже
суженный Pass 1 список вместо полного. Переключение языка (`l`) происходит
один раз, сразу после сброса, — `l` переводит весь интерфейс целиком (не
только Help), поэтому весь Pass 2 идёт на русском; обратного переключения в
EN нет, GIF заканчивается на последнем экране Pass 2.
...
- `notes.yaml` — вся проза документа: примечания по моделям, обоснования фаворитов,
  ...
  `task_fit` для рабочих задач. Поле `note` у модели — единственное двуязычное: либо
  обычная строка (устаревший формат, трактуется как русский текст с пустым переводом),
  либо `{ru, en}`, где `ru` остаётся каноническим текстом для документа и CLI, а `en` —
  перевод для TUI; пустая строка или отсутствующий ключ `en` значит «ещё не переведено».
...
## Выделение текста в `omt tui`

В TUI зажмите левую кнопку мыши и перетащите курсор по непустому видимому тексту.
При включённом terminal mouse reporting и доставленном событии release после отпускания
выделение копируется в системный clipboard. Клавиша `y`
повторно копирует сохранённое выделение. Оно переживает redraw, анимацию,
длительный refresh, async-результат clipboard и завершение toast; release
копирует snapshot кадра, на котором началось выделение, без ANSI-кодов и правого
padding.

Новое выделение, клавиатурная навигация и переход в новый экран сбрасывают старое
выделение. В `help`, `detail`, `columns`, `settings` и `filter` мышь выделяет
текст текущего окна. Поля ввода сохраняют обычную обработку клавиш.

Копирование сначала использует `atotto/clipboard`, затем отправляет OSC52 как
fallback, если terminal clipboard backend недоступен и терминал поддерживает OSC52;
внутри tmux сохраняется passthrough-обёртка. Успех OSC52 зависит от поддержки терминалом
и доступности writer'а. OSC52-последовательность выводится через общий synchronized writer,
переданный и Bubble Tea renderer, и clipboard fallback: отдельные вызовы `Write` не
перемешиваются. Это не обещает атомарность на уровне терминала или гарантию доставки
командой-обёрткой. Если terminal mouse reporting, release, непустая видимая область или
clipboard backend/OSC52 недоступны, копирование не гарантируется и TUI показывает результат
в status.

## Task-fit-фильтр

Внутри одного `task_fit:K1,K2,...` значения объединяются через OR. Повторные `task_fit:` предикаты объединяются через AND. Допустимы `implement`, `plan`, `research`, `debug`, `audit`, `refactor`, `test`; `task_fit:` выбирает модели без назначенного task fit. Все остальные предикаты также объединяются через AND.

## Tier-фильтр

`tier:VALUE[,VALUE...]` означает точное членство в одном или нескольких tiers (`tier:opus,haiku` — Opus или Haiku). `(any)` в TUI означает отсутствие tier-предиката. Значение `free` остаётся допустимым в сохранённом/CLI-синтаксисе и выбирает только `free`, но не предлагается среди видимых чипов TUI.

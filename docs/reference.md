...
`task_fit:K1,K2,...` требует все перечисленные ключевые слова. Допустимы `implement`, `plan`, `research`, `debug`, `audit`, `refactor`, `test`; `task_fit:` выбирает модели без назначенного task fit. Несколько предикатов `task_fit:` объединяются через OR, а все остальные фильтры объединяются с результатом task-fit через AND.

## Tier-фильтр

`tier:MIN` означает минимальный платный tier: selected tier включается вместе со всеми более высокими (`tier:opus` — только Opus, `tier:sonnet` — Opus и Sonnet, `tier:haiku` — Opus, Sonnet и Haiku). `tier:free` сохраняет legacy exact semantics и выбирает только `free`, не paid tiers. Значение `free` остаётся допустимым в сохранённом/CLI-синтаксисе, но не предлагается как вариант минимального tier в TUI.

...
- `openrouter history [--model SLUG] [--since RFC3339|YYYY-MM-DD] [--format markdown|tsv|report]` — показать историю цен. `markdown`/`tsv` — полный журнал изменений по всем моделям (или по одной при `--model`), построчно. `report` — читаемый отчёт по ОДНОЙ модели (обязателен `--model`): дедуплицированная таблица (один ряд на отдельную цену, с диапазоном дат и числом дней вместо повторяющейся строки на каждое ежедневное наблюдение) и подписанный столбчатый график по каждому из input/output рядов с осью $ и датами. Ширина графика подстраивается под реальный терминал, если stdout — TTY, иначе использует `pricehistory.DefaultChartWidth`.

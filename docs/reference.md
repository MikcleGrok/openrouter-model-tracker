...
| `profiles` | `active`: plain CLI/TUI, build/release и supply-chain, **и** publishable (см. `channels`) — оба применимы одновременно, publishable задаёт более строгую SCA cadence; `N/A`: daemon, container runtime |
...
| `Makefile targets` | baseline: `check` (SCA-freshness staleness gate), `fmt-check`, `test-unit`, `test-acceptance`, `vet`, `security`, `dependency-check`, `secrets-check`, `sbom`, `check-docs`; domain: `cli-check`; release: `release-check`, `release-manifest`, `sign`, `attest`, `verify-provenance`, `checksums`, `verify-release`, `distribution-check` |
...
| `review trigger/profile state` | active; пересмотр при изменении release channel, version source, signing/provenance, trust boundary или не позднее 2026-12-08 |

...
`docs/assets/tui-demo.gif` — шесть частей (совпадают с комментариями `# 1.`–`# 6.`
в самом `docs/assets/tui-demo.tape`):
...
6. переключение языка (`l`) — список, затем вкладка Identity экрана Detail и
   секция Overview экрана Help, показывая, что `l` переводит весь интерфейс,
   а не только справку; `l` снова возвращает EN

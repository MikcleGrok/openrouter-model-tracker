...
Обновлено: 2026-08-07 (цены и контекст получены из каталога OpenRouter (/api/v1/models), оценки — с vals.ai по ручной карте model-map.tsv (источник swebench.com подключён, но пока не даёт ни одной строки — в карте нет ни одной записи swebench=); остальные числа заданы вручную в notes.yaml и помечены как вендорские)
...
| ↳ второй выбор | GPT-5.6 Sol | $5.00 / $30.00 ($10.00 / $45.00 от 272K+) · 1.1M | 96.2% | 8.6 | OpenAI (C) | нет | Заметно хуже по цена/качество, чем Luna, но ближе всего к Opus 5 по сырой оценке. METR фиксировал эксплуатацию багов eval'ов у Sol в **другом** бенчмарке, а не в SWE-bench Verified — но общая осторожность к её eval-результатам сохраняется. |
| ≈ Sonnet 5 | MiniMax M3 | $0.30 / $1.20 · 1M | 80.5% (только вендор) | 153 | MiniMax (н/д) | **да** (кастомная лицензия, коммерч. ограничения) | Топ-1 по цена/качество в Sonnet-тире при цене в 10-12 раз ниже полного прайса Sonnet 5. Оговорка: 80.5% — только по блогу вендора, на независимых лидербордах модели нет. |
| ↳ второй выбор | Meta Muse Spark 1.1 | $1.25 / $4.25 · 1M | 77.4% (только вендор) | 38.7 | Meta (D+) | нет | Следующая по «качество/цена» среди строк с оценкой именно этого продукта; её 77.4% тоже вендорские, но относятся к ней самой. |
| <≈ Haiku 4.5 | DeepSeek V4 Flash | $0.0882 / $0.1764 · 1M | SWE-V 73.7–79.0% (только вендор) | 693 | DeepSeek (F) | **да, MIT** | Лучшая цена/качество во всём тире <≈ Claude Haiku 4.5: SWE-V 73.7–79.0% (вендорские цифры по двум вариантам линейки) при цене вне конкуренции с остальными строками тира — независимо от того, какая из двух оценок точнее. |
| ↳ второй выбор | Tencent Hy3 | $0.132 / $0.528 · 262K | SWE-V 78.0% (только вендор) | 338 | Tencent / Hunyuan (н/д) | **да, Apache 2.0** | Следующая по цена/качество среди строк с самоотчётной, но правдоподобной оценкой (78.0%, только вендор, достоверность низкая); уже заметно используется в проде — #2 по объёму трафика и #1 по числу tool-calls в категории "coding" на OpenRouter. |
| <≈ Haiku 4.5 (бесплатная) | Poolside Laguna XS 2.1 | $0 / $0 · 262K | SWE-bench Verified 70.9% (только вендор) | н/д (цена $0) | Poolside | да, OpenMDW-1.1 | Впереди по сырой оценке (70.9% против 70.4% у Nemotron 3 Ultra), но это одиночная точка по данным самого вендора (Poolside прямо пишет в карточке, что публикует «official scores» из своих релизных постов) — не подтверждена независимо. Контекст меньше, чем у Nemotron 3 Ultra (262K против 1M). |
| ↳ второй выбор | NVIDIA Nemotron 3 Ultra | $0 / $0 · 1M | 65–70.4% (5 скаффолдов, прогон самой NVIDIA) | н/д (цена $0) | NVIDIA | да, OpenMDW-1.1 | Лимиты бесплатного тира одинаковы для всех `:free`-моделей, так что отбор фактически сводится к качеству и близким числам. По сырой оценке немного отстаёт от Laguna XS 2.1 (70.4% против 70.9%), но диапазон получен на 5 харнессах (устойчивее одиночной самоотчётной точки), а контекст — 1M против 262K у Laguna: близкий, хорошо обоснованный второй выбор. |
...
| openai/gpt-5.6-terra | openai/gpt-5.6-terra | $1.00 ($2.00 от 272K+) | $6.00 ($9.00 от 272K+) | 1.1M | н/д | н/д (нет оценки по SWE-bench Verified) | _нужен обзор_ | _нужен обзор_ | _нужен обзор_ |
...
| Модель | Slug на OpenRouter | Вход $/M | Выход $/M | Контекст | SWE-bench Verified | Качество/цена | Владелец (FLI) | Открытые веса | Примечание |
|---|---|---|---|---|---|---|---|---|---|
| MiniMax M3 | minimax/minimax-m3 | $0.30 | $1.20 | 1M | 80.5% (только вендор) | 153 | MiniMax (н/д) | **да** (кастомная лицензия, коммерч. ограничения) | Лучшая цена/качество в этом тире. Оценка — только из блога вендора (minimax.io); на независимых лидербордах модели нет, достоверность низкая. У GMICloud есть маршрут дешевле типовой цены каталога. |
| Meta Muse Spark 1.1 | meta/muse-spark-1.1 | $1.25 | $4.25 | 1M | 77.4% (только вендор) | 38.7 | Meta (D+) | нет | Флагман Meta вместо бренда Llama; 77.4% — заявка самой Meta, независимого подтверждения нет. Цифру SWE-bench Verified Hard 42.9% **не удалось проверить на 2026-07-30** (исходный PDF не читается, часть обзоров её вообще не упоминает) — оставлена с прошлой версии; сторонние обзоры дают только SWE-bench Pro 52.4–61.5% и Terminal-Bench. |
| Qwen3.7 Max | qwen/qwen3.7-max | $1.475 | $4.425 | 1M | 80.4% (только вендор) | 36.3 | Alibaba Cloud / Qwen (D−) | нет | 80.4% держится только на вендорских и около-вендорских публикациях, независимого лидерборда нет. Alibaba анонсировала превью Qwen 3.8-Max (2026-07-19); к 2026-07-30 у превью появились API и цена ($0.17/$0.51 за M — «10% от стандартной»), но SWE-bench Verified не публиковали и продукта на OpenRouter нет — рано менять рекомендацию. |
...
| meta-llama/llama-4-scout | meta-llama/llama-4-scout | $0.10 | $0.30 | 1.3M | н/д | н/д (нет оценки по SWE-bench Verified) | _нужен обзор_ | _нужен обзор_ | _нужен обзор_ |
...
| Llama 4 Maverick | meta-llama/llama-4-maverick | $0.20 | $0.80 | 1M | н/д | н/д (нет оценки по SWE-bench Verified) | Meta (D+) | **да** (Llama 4 Community License, ограничения при >700M MAU) | Старое поколение (апрель 2025), больше не флагман Meta (теперь — Muse Spark). Достоверной оценки SWE-bench Verified не нашли и в этой проверке (циркулируют ~24%, 49.2%, а 8.0% — вообще по SWE-bench **Lite**). |
...
| nvidia/nemotron-3-super-120b-a12b | nvidia/nemotron-3-super-120b-a12b | $0.30 | $0.90 | 1M | н/д | н/д (нет оценки по SWE-bench Verified) | _нужен обзор_ | _нужен обзор_ | _нужен обзор_ |
...
| GPT-5 Mini | openai/gpt-5-mini | $0.25 | $2.00 | 400K | н/д | н/д (нет оценки по SWE-bench Verified) | OpenAI (C) | нет | Старое поколение — актуального "mini" в линейке 5.6 нет; ближайший бюджетный аналог сейчас — GPT-5.6 Luna в тире Opus выше. Сводимой оценки нет и в этой проверке (38% и 48% из источников без ссылок). |
| Mistral Large 3 | mistralai/mistral-large-2512 | $0.50 | $1.50 | 262K | н/д | н/д (нет оценки по SWE-bench Verified) | Mistral AI (F) | **да, Apache 2.0** | Универсальный флагман Mistral; преемника (Large 4) на дату проверки не существует, анонсов нет. Оценки SWE-bench Verified не найдено нигде. |
...
| nvidia/nemotron-3-ultra-550b-a55b | nvidia/nemotron-3-ultra-550b-a55b | $0.60 | $3.60 | 512K | н/д | н/д (нет оценки по SWE-bench Verified) | _нужен обзор_ | _нужен обзор_ | _нужен обзор_ |
...
| Модель | Чисто вход (M на $10) | Чисто выход (M на $10) | Смешанный (M на $10) |
|---|---|---|---|
| DeepSeek V4 Flash | 113 | 56.7 | 90.7 |
...
| nvidia/nemotron-3-super-120b-a12b | 33.3 | 11.1 | 22.2 |
...
- Значительная часть оценок в таблицах — **вендорские** (помечено «только вендор»). Независимо подтверждены только те строки, число которых пришло с vals.ai (источник swebench.com в код подключён, но в model-map.tsv пока нет ни одной записи swebench=, поэтому он сегодня не даёт ни одной строки).
...
| NVIDIA Nemotron 3 Ultra | `nvidia/nemotron-3-ultra-550b-a55b:free` | 1M | 65–70.4% (5 скаффолдов, прогон самой NVIDIA) | <≈ Haiku 4.5 (середина диапазона) | NVIDIA | да, OpenMDW-1.1 | 550B/55B-active MoE. Прогон на 5 разных агентных скаффолдах делала сама NVIDIA — это демонстрация устойчивости к харнессу, а не независимое подтверждение (сторонней оценки SWE-bench Verified у модели нет). |

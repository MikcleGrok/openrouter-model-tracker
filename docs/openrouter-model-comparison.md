...
Обновлено: 2026-08-10 (цены и контекст получены из каталога OpenRouter (/api/v1/models), оценки — с vals.ai и swebench.com по ручной карте model-map.tsv (21 запись vals=, 6 записей swebench=); остальные числа заданы вручную в notes.yaml и помечены как вендорские)
...
| ≈ Sonnet 5 | GLM-5.2 | $0.07 / $0.22 · 1M | 82.8% | 770 | Z.ai / Zhipu AI (D−) | **да, MIT** | _нужен обзор_ |
...
| Модель | Slug на OpenRouter | Вход $/M | Выход $/M | Контекст | SWE-bench Verified | Качество/цена | Владелец (FLI) | Открытые веса | Примечание |
|---|---|---|---|---|---|---|---|---|---|
| GLM-5.2 | z-ai/glm-5.2 | $0.07 | $0.22 | 1M | 82.8% | 770 | Z.ai / Zhipu AI (D−) | **да, MIT** | Официальная документация Z.AI SWE-bench Verified не публикует — там только SWE-bench Pro 62.1% и Terminal-Bench 2.1 81.0% (другие метрики). Независимая оценка SWE-bench Verified есть на vals.ai (82.8%). Модель по-прежнему сильна в агентных задачах. |

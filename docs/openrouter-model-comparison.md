...
| ≈ Sonnet 5 | MiniMax M3 | $0.30 / $1.20 · 1M | 75.0% | 143 | MiniMax (н/д) | **да** (кастомная лицензия, коммерч. ограничения) | Топ-1 по цена/качество в Sonnet-тире при цене в 10-12 раз ниже полного прайса Sonnet 5. Оговорка: 80.5% — только по блогу вендора, на независимых лидербордах модели нет. |
| ↳ второй выбор | DeepSeek V4 Pro | $0.44 / $0.87 · 1M | 77.4% | 142 | DeepSeek (F) | **да, MIT** | _нужен обзор_ |
...
| GLM-5.2 | z-ai/glm-5.2 | $0.76 | $2.42 | 1M | 82.8% | 70.5 | Z.ai / Zhipu AI (D−) | **да, MIT** | Официальная документация Z.AI SWE-bench Verified не публикует — там только SWE-bench Pro 62.1% и Terminal-Bench 2.1 81.0% (другие метрики). Независимая оценка SWE-bench Verified есть на vals.ai (82.8%). Модель по-прежнему сильна в агентных задачах. |
...
| Llama 4 Maverick | meta-llama/llama-4-maverick | $0.20 | $0.70 | 1M | 21.0% | 64.9 | Meta (D+) | **да** (Llama 4 Community License, ограничения при >700M MAU) | Старое поколение (апрель 2025), больше не флагман Meta (теперь — Muse Spark). Независимая оценка SWE-bench Verified теперь есть на swebench.com (21.0%) — заметно ниже циркулировавших ранее непроверенных чисел (~24%, 49.2%, а 8.0% — вообще по SWE-bench **Lite**). |
...
| google/gemma-4-26b-a4b-it | google/gemma-4-26b-a4b-it | $0.12 | $0.40 | 262K | н/д | н/д (нет оценки по SWE-bench Verified) | _нужен обзор_ | _нужен обзор_ | _нужен обзор_ |

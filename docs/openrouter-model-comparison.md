...
Обновлено: 2026-08-08 (цены и контекст получены из каталога OpenRouter (/api/v1/models), оценки — с vals.ai по ручной карте model-map.tsv (источник swebench.com подключён, но пока не даёт ни одной строки — в карте нет ни одной записи swebench=); остальные числа заданы вручную в notes.yaml и помечены как вендорские)
...
| Qwen3.7 Max | qwen/qwen3.7-max | $1.48 | $4.43 | 1M | 80.4% (только вендор) | 36.3 | Alibaba Cloud / Qwen (D−) | нет | 80.4% держится только на вендорских и около-вендорских публикациях, независимого лидерборда нет. Alibaba анонсировала превью Qwen 3.8-Max (2026-07-19); к 2026-07-30 у превью появились API и цена ($0.17/$0.51 за M — «10% от стандартной»), но SWE-bench Verified не публиковали и продукта на OpenRouter нет — рано менять рекомендацию. |

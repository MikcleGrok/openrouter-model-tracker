...
- `make verify-local-artifact` теперь сначала регенерирует `.release/manifest.json` и
  ...
  также digest самого локального бинарника. `.release/manifest.json` сам нигде не
  подписывается и не публикуется. Его checksum (`.release/openrouter.sha256`)
  публикуется как GitHub Release asset вместе с остальными artifacts (`release-github`,
  Makefile), но не подписывается — в отличие от `.release/release-manifest.json`,
  который несёт криптографическую подпись поверх своих текущих байт и который
  `release-github-check` никогда не регенерирует автоматически. Регенерация checksum
  через `manifest` остаётся безопасной несмотря на публикацию: это детерминированный
  `shasum` от детерминированно пересобранного бинарника, поэтому повторный запуск
  даёт байт-идентичный файл, а любое расхождение с реально опубликованным релизом
  отдельно ловит `make verify-provenance` — он валидирует published evidence
  (включая этот checksum) против фактически опубликованных artifacts.
...
Winget-манифесты (`make winget-manifest`) генерируются в этом репозитории, но
публикуются PR в `microsoft/winget-pkgs` — внешний репозиторий, мёржит который
внешний модератор по своему усмотрению. Этот репозиторий не даёт никаких
гарантий насчёт того, что реально раздаётся через winget после мёржа: ни
таймлайн ревью, ни решение модератора, ни последующие правки самого манифеста
в `winget-pkgs` этому репозиторию не подконтрольны.

Единственное, что реально проверяется здесь: `InstallerSha256`, отправляемый в
манифесте, совпадает с digest из `SHA256SUMS`/`manifest.json` — это проверяет
`make winget-submit-check` до открытия любого PR, и тот же порядок закрыт
механически (`BLOCKED:` до `release-github`, см.
[docs/reference.md](reference.md#winget-submission)).

Принятый риск — Windows `.exe` внутри архива не подписан ни Authenticode, ни
cosign (code signing явно вне рамок, см. выше). Валидационный CI
`winget-pkgs` реально запускает установщик, и Defender может пометить
неподписанный/малоизвестный Go-бинарник ложноположительно — известное,
нефатальное, видимое модератору состояние (label `Validation-Defender-Error`
в `winget-pkgs`). Митигация — репорт false positive в Microsoft
(`aka.ms/wdsi`) и заметка в PR, а не добавление подписи.

`make winget-submit` использует только обычный интерактивный `gh auth login`
со scope `public_repo` — новый секрет не добавляется, cosign-ключ этим flow не
затрагивается.

## Scoop-публикация

Scoop-манифест (`make scoop-manifest`) генерируется в этом репозитории и
пишется напрямую в bucket-репозиторий, которым мы владеем сами
(`MikcleGrok/scoop-bucket`). В отличие от `winget-pkgs`, здесь нет внешнего
модератора — но, в отличие от `winget-pkgs`, у которого есть собственная
валидационная CI, реально запускающая установщик, здесь нет и внешнего
валидационного пайплайна вообще. Единственная проверка, которая существует, —
собственный `make scoop-submit-check` этого репозитория.

Он проверяет, что `hash`, `version` и `url` в сгенерированном манифесте
совпадают с digest из `SHA256SUMS`/`manifest.json` и с опубликованным GitHub
Release — до того, как в bucket-репозиторий уйдёт хоть одна запись (см.
«Строгий порядок» в [docs/reference.md](reference.md#scoop-bucket)).

Ничего из этого не запускает установщик на настоящей Windows: Windows CI в
этом репозитории нет вообще, точно как и для winget, поэтому реальная
установка через `scoop install` не проверяется автоматически ни здесь, ни
где-либо ещё. Windows `.exe` внутри архива остаётся неподписанным — тот же
принятый риск, что и для winget, этот канал его не меняет. Алиас `omt` в
`bin`-секции манифеста не проверен end-to-end ни на одной реальной
Windows-машине.

`make scoop-submit` использует тот же обычный интерактивный `gh auth login`
со scope `public_repo`, уже требуемый для сабмита winget-манифеста, — новый
секрет не добавляется, cosign-ключ этим flow не затрагивается; право записи в
bucket-репозиторий проистекает из того, что аутентифицированный пользователь
сам им владеет.

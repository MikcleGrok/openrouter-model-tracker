...
- В TUI источник оценки переключается без изменения keybinding: нажмите `o`, стрелкой `Down` перейдите на `Score source`, затем нажмите `Enter` или `Space`; это переключает `SWE-bench` и `Arena`. Текущий источник виден в meta-строке, Settings и status hints.

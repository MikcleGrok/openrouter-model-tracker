...
Homebrew устанавливает canonical `openrouter-model-tracker` и короткий alias
`omt` как один и тот же executable (проверено по формуле:
`bin.install ... => "openrouter-model-tracker"` +
`bin.install_symlink "openrouter-model-tracker" => "omt"`) — другое имя
canonical-бинарника, чем у локальной установки через `make install` ниже
(`openrouter`/`omt`), потому что общий tap `mikclegrok/tools` хостит несколько
инструментов и `openrouter` как short name там был бы коллизионным. Оба имени
печатают одну и ту же версию (`<bin> version` → `openrouter X.Y.Z`) — это
внутренняя версия CLI, а не имя файла. Обновление до новой версии:

...
Колонка `Claude` — относительная оценка к Haiku 4.5. Для `opus` и `sonnet` выводится `>≈ Haiku 4.5`;
для `haiku` и `free` с rankable Score применяются пороги 70 и 60, а без score используются fallback
`≈ Haiku 4.5` для `haiku` и `<≈ Haiku 4.5` для `free`. Неизвестный tier отображается как `н/д`.

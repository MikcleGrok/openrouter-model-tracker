...
`table.icon_gap` задаёт отступ между иконкой производителя и его именем и
применяется одинаково в CLI и TUI. По умолчанию это 1 пробел; принимаются
...
`table.icon_gaps` переопределяет gap только на границе `icon -> manufacturer`;
граница `manufacturer -> model`, padding ячейки и `table.name_width` не меняются.
По умолчанию Meta и Mistral используют 2 пробела, остальные производители —
глобальный `icon_gap` (1). Значения override также ограничены диапазоном 0..8;

...
Справка TUI занимает весь viewport и прокручивается клавишами ↑/↓ (или `j`/`k`), `Home`/`End` (или `g`/`G`) и `PgUp`/`PgDown`; `/` открывает поиск по тексту справки: набираемый запрос сразу виден отдельной строкой `/ …_` внизу экрана, как в основном списке. `Enter` переходит к следующему совпадению, а `Esc` или `?` закрывают справку. В верхней строке TUI показывается RFC3339-время последнего успешного обновления данных.

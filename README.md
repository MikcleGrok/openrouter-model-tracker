...
Для `opus` выводится `>≈ Claude Opus 5`, для `sonnet` — `≈ Claude Sonnet 5`; для `haiku` и `free`
с rankable Score применяются пороги 70 и 60 относительно `Claude Haiku 4.5`, а без score используются
fallback `≈ Claude Haiku 4.5` для `haiku` и `<≈ Claude Haiku 4.5` для `free`. Неизвестный tier

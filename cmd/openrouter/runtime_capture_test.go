...
func TestTUIRuntimeCaptureAcrossRealSession(t *testing.T) {
	...
	step("открыт detail", detailWithFooter("Detail 1-18/18"))

	sess.Resize(90, 24)
	rp.Send(tea.WindowSizeMsg{Width: 90, Height: 24})
	step("resize при открытом detail", detailWithFooter("Detail 1-18/18"))
	...
	rp.Send(tea.KeyMsg{Type: tea.KeyEnter})
	step("повторно открыт detail", detailWithFooter("Detail 1-18/18"))

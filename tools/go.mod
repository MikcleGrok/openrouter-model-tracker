// Pinned dev/security tooling for the Makefile's `dependency-check` gate.
//
// This lives in its own module, not in the root go.mod, on purpose. A tool
// directive puts the tool's whole transitive dependency graph into the module
// that declares it, and the root module's graph is exactly what
// `make dependency-check` scans and what the shipped binary is built from.
// Declaring the scanners at the root would make the scanners' own supply
// chain part of the scanned dependency state: measured on this repo, the root
// module's graph is 39 modules; osv-scanner and govulncheck alone pull in 474
// (deps.dev clients, a SQLite driver, OpenTelemetry, gRPC, and more) — none of
// it linked into the shipped `openrouter` binary, all of it noise `make
// dependency-check` would otherwise have to scan and `make check`'s staleness
// gate would have to treat as part of this project's own dependency state.
//
// Isolating them here keeps every guarantee 05-build-test-docs.md's "Версии
// dev-инструментов" section asks for: each tool is pinned by exact version in
// a committed go.mod/go.sum, resolved from that manifest and never from PATH,
// and built and run by the same Go toolchain as the project (`go -C tools
// tool <name>`), which is the export-data compatibility the rule exists to
// protect.
module github.com/sboborikin/openrouter-model-tracker/tools
